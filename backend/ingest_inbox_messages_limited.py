...
def create_inbox_documents(loader: BubbleDataLoader, limit: int = 50) -> List[Document]:
    """Create documents from inbox messages (limited for testing)."""
    documents = []
    ...
    users = {}
    try:
        user_data = loader.load_data("user", limit=200, fields=["_id", "Name", "email"])
    ...
    events = {}
    try:
        event_data = loader.load_data("event", limit=200, fields=["_id", "Event Code", "Event Name"])
    ...
    logger.info(f"Loading InboxConversation data (limited to {limit})...")
    try:
        conversations = loader.load_data("InboxConversation", limit=limit)
        ...
        if conversations:
            ...
            for i, conv in enumerate(conversations[:3]):
                ...
                logger.info(f"  {i+1}. {subject} (Status: {status})")
    ...
    logger.info(f"Loading InboxConversationUser data (limited to {limit})...")
    try:
        conv_users = loader.load_data("InboxConversationUser", limit=limit)
...
def main():
    ...
    documents = create_inbox_documents(loader, limit=50)
    ...
    if event_codes:
        logger.info(f"\nFound messages for {len(event_codes)} different events")
        logger.info("Sample event codes: " + ", ".join(list(event_codes)[:10]))
